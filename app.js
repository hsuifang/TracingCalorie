...
const ItemCtrl = (function(){
  ...
  const data = {
    ...
    // ],
    items: StorageCtrl.getItemFromStroage(),
    currentItem: null,
    totalCalories: 0
  }
  // Public methods
  return {
    ...
    addItem: function (name, calories) {
      ...
      if (data.items.length > 0) {
        // ID = Math.max.apply(null, data.items.map(x => x.id))
        ID = Math.max(...data.items.map(x => x.id)) + 1
      } else {
        ID = 0
      }

      // Calories to number
      const parseCalorie = parseInt(calories)

      // Create new item
      newItem = new Item(ID, name, parseCalorie)
...
const App = (function(ItemCtrl, StorageCtrl, UICtrl){
  ...
  const itemUpdateSubmit = function (e) {
    ...
    // Update item
    const updateItem = ItemCtrl.updateItems(input.name, input.calories)
